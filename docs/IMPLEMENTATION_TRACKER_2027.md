...
| [H7 uncertainty handoff](REUSE_RECOMMENDATIONS_2027.md#acceptance-h7) | Ready for review | Pass each camera measurement's standard deviations from `RobotState` to WPILib's pose estimator. | A temporary desktop diagnostic gave 0.455 m correction with low uncertainty and 0.005 m with high uncertainty for the same 0.5 m camera offset; infinite heading uncertainty left heading unchanged. The diagnostic was removed. `spotlessCheck build` passed. | Mentor reviews the handoff. Camera parsing, stream policy, and live robot checks remain separate H7 work. |
